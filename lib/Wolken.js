...
class Wolken {
     ...
     * @param {Boolean|String} [options.nsfw] Whether to allow NSFW results. If this is a string, it should be "only".
    ...
    getRandom(options={}) {
        return new Promise((resolve, reject) => {
            ...
            if (options.nsfw && typeof options.nsfw !== 'boolean' && options.nsfw !== 'only') throw new TypeError('options.nsfw is not a boolean, or not "only"');
            ...
            let query = `?hidden=${this.allowHidden}&nsfw=${options.nsfw || false}`;
